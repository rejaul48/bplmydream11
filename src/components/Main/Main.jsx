            ...
            <main className='max-w-[1280px] mx-auto mt-12 pb-6'>
                <div className='main_section_head md:flex  justify-between items-center px-4 xl:px-0'>
                    ...
                    <div className='avialable_and_selected_btn border-2 rounded-lg mt-3 md:mt-0'>
                        <button onClick={() => { handleToggleBtn("available") }} className={`btn  w-[50%] md:w-auto text-xs md:text-lg ${toggleBtn === "available" ? 'bg-lime-400' : 'bg-none'} `}>Available</button>
                        <button onClick={() => { handleToggleBtn("selected") }} className={`btn w-[50%] md:w-auto text-xs md:text-lg ${toggleBtn === "selected" ? 'bg-lime-400' : 'bg-none'} `}>Selected ( <span>{selectPlayer.length}</span> )</button>
